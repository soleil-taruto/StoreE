...
namespace Charlotte
{
	class Program
	{
		...
		private void MakeCatalogFile(string outputDir, string catalogFile)
		{
			...
			if (Directory.Exists(catalogFile))
				throw new Exception("catalogFile is not creatable file");
		...
		private void MakeDifferenceDir(string inputDir, string catalogFile, string differenceDir)
		{
			...
			if (File.Exists(differenceDir))
				throw new Exception("differenceDir is not creatable dir");
