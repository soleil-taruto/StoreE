...
{
	public class Enemy_B神奈子0001 : TVEnemy
	{
		...
		protected override IEnumerable<bool> E_Draw()
		{
			...
			for (int frame = 0; ; frame++)
			{
				if (!this.SpecialDraw())
				{
					...
					x *= TopView.I.Map.W * TopViewConsts.TILE_W;
					y *= TopView.I.Map.H * TopViewConsts.TILE_H;
					...
					if (frame % 30 == 0)
					{
						TopView.I.Enemies.Add(new Enemy_B神奈子_Tama(this.X, this.Y));
					}
		...
		protected override void P_Damaged(TVShot shot, int damagePoint)
		{
			this.SpecialDraw = SCommon.Supplier(this.E_HitBack());
			TVEnemyCommon.Damaged(this, shot, damagePoint);
			TopView.I.Enemies.Add(new Enemy_B神奈子_Tama(this.X, this.Y)); // 撃ち返し
		}
		...
		{
			敵弾一掃();
			TopView.I.Enemies.Add(new Enemy_B神奈子9901(this.X, this.Y));
		}

		private static void 敵弾一掃()
		{
			foreach (TVEnemy enemy in TopView.I.Enemies.Iterate())
