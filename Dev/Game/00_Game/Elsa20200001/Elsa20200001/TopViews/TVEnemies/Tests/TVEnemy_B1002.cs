...
{
	public class TVEnemy_B1002 : TVEnemy
	{
		private double XAdd;
		private double YAdd;

		public TVEnemy_B1002(double x, double y, double xAdd, double yAdd)
			: base(x, y, 1, 1, true)
		{
			this.XAdd = xAdd;
			this.YAdd = yAdd;
		...
		protected override IEnumerable<bool> E_Draw()
		{
			...
			for (; ; )
			{
				...
				DDUtils.MakeXYSpeed(this.X, this.Y, TopView.I.Player.X, TopView.I.Player.Y, 0.3, out accelX, out accelY);
				...
				if (TopView.I.Map.GetCell(TopViewCommon.ToTablePoint(this.X - R, this.Y)).Tile.GetKind() == TVTile.Kind_e.WALL)
				{
				...
				if (TopView.I.Map.GetCell(TopViewCommon.ToTablePoint(this.X, this.Y - R)).Tile.GetKind() == TVTile.Kind_e.WALL)
				{
