...
namespace Charlotte.TopViews.TVEnemies
{
	...
	public abstract class TVEnemy
	{
		...
		public TVEnemy(double x, double y, int hp, int attackPoint, bool 自機に当たると消滅する)
		{
			this.X = x;
			this.Y = y;
			this.HP = hp;
		...
		public void Damaged(TVShot shot, int damagePoint)
		{
		...
		/// <param name="shot">この敵が被弾したプレイヤーの弾</param>
		/// <param name="damagePoint">削られた体力</param>
		protected virtual void P_Damaged(TVShot shot, int damagePoint)
		{
		...
		{
			TVEnemyCommon.Killed(this, destroyed);
		}
	}
}
