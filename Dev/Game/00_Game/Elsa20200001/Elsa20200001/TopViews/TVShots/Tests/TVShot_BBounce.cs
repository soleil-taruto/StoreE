...
namespace Charlotte.TopViews.TVShots.Tests
{
	...
	/// テスト用_自弾
	/// </summary>
	public class TVShot_BBounce : TVShot
	{
		private int Direction; // この自弾の進行方向(8方向_テンキー方式)

		public TVShot_BBounce(double x, double y, int direction)
			: base(x, y, 1, false)
		{
		...
		{
			D2Point speed = TopViewCommon.GetXYSpeed(this.Direction, 8.0);
			int bouncedCount = 0;
			...
			for (int frame = 0; ; frame++)
			{
				...
				if (bounced) // ? 壁に当たった。
				{
					...
					{
						DDGround.EL.Add(SCommon.Supplier(TVEffects.B跳ねた(this.X, this.Y)));
						bouncedCount++;
					}
					else // ? 跳ね返り不能
					{
		...
		{
			int x = SCommon.ToInt(this.X + xa) / TopViewConsts.TILE_W;
			int y = SCommon.ToInt(this.Y + ya) / TopViewConsts.TILE_H;

			return TopView.I.Map.GetCell(x, y).Tile.GetKind() == TVTile.Kind_e.WALL;
		}
	}
}
