...
{
	public abstract class TVTile_大森林系 : TVTile
	{
		...
		public TVTile_大森林系(DDPicture[,] pictbl, int 密集_l, int 密集_t, int 単独_l, int 単独_t, DDPicture groundPicture, DDPicture 単独セルPicture)
		{
			_draw = new DrawTask()
			{
				PictureTable = pictbl,
		...
		private class DrawTask : DDTask
		{
			...
			public override IEnumerable<bool> E_Task()
			{
				for (; ; )
				{
					...
					else
					{
						...
						{
							DDDraw.SetMosaic();
							DDDraw.DrawBegin(picture, this.DrawPoint.X - TopViewConsts.TILE_W / 2, this.DrawPoint.Y - TopViewConsts.TILE_H / 2);
							DDDraw.DrawZoom(2.0);
							DDDraw.DrawEnd();
							DDDraw.Reset();
						}
			...
			{
				return this.F_IsFriend(TopView.I.Map.GetCell(x, y));
			}
		}
	}
}
