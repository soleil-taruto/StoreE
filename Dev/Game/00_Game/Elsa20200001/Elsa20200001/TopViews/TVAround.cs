...
namespace Charlotte.TopViews
{
	public class TVAround
	{
		...
		public TVAround(int x, int y, int size)
		{
			I2Point pt = new I2Point(x, y);

			this.Size = size;
			this.Table = new TVMapCell[size, size];

			// 周辺テーブルの左上へ移動
			x -= (size - 1) * TopViewConsts.TILE_W / 2;
			y -= (size - 1) * TopViewConsts.TILE_H / 2;

			const int TMP_SPAN = 1000;

			// マップ座標(ドット単位) -> マップテーブル座標
			x += TopViewConsts.TILE_W * TMP_SPAN;
			y += TopViewConsts.TILE_W * TMP_SPAN;
			x /= TopViewConsts.TILE_W;
			y /= TopViewConsts.TILE_H;
			x -= TMP_SPAN;
			y -= TMP_SPAN;

			for (int xc = 0; xc < size; xc++)
				for (int yc = 0; yc < size; yc++)
					this.Table[xc, yc] = TopView.I.Map.GetCell(x + xc, y + yc);
			...
			// 周辺テーブルの中心へ移動
			x += size * TopViewConsts.TILE_W / 2;
			y += size * TopViewConsts.TILE_H / 2;
		...
		{
			TVMapCell tmp = this.Table[x1, y1];
