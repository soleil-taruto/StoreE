...
{
	public static class TopViewCommon
	{
		// ===========================
		// ==== Map 関連 (ここから) ====
		// ===========================
		...
		{
			return new I2Point(
				(int)Math.Floor(x / TopViewConsts.TILE_W),
				(int)Math.Floor(y / TopViewConsts.TILE_H)
				);
		}
		...
		{
			return new D2Point(
				(double)(x * TopViewConsts.TILE_W + TopViewConsts.TILE_W / 2.0),
				(double)(y * TopViewConsts.TILE_H + TopViewConsts.TILE_H / 2.0)
				);
		}
		...
		private static TVMapCell _defaultMapCell = null;
		...
		/// </summary>
		public static TVMapCell DefaultMapCell
		{
			get
			{
				if (_defaultMapCell == null)
				{
					_defaultMapCell = new TVMapCell()
					{
						TileName = TopViewConsts.TILE_NONE,
						Tile = new TVTile_None(),
						EnemyName = TopViewConsts.ENEMY_NONE,
					};
				}
		...
		{
			GameStatus gameStatus = TopView.I.Status.GetClone();
			...
			// -- マップ退場時
			{
				...
				gameStatus.TVStartHP = TopView.I.Player.HP;
				gameStatus.TVStartFaceDirection = TopView.I.Player.FaceDirection;
				gameStatus.TVStart選択武器 = TopView.I.Player.選択武器;
			}
		...
		public static void SaveGame(GameStatus gameStatus)
		{
			...
			for (; ; )
			{
				...
				if (selectIndex < TopViewConsts.SAVE_DATA_SLOT_NUM)
				{
					if (new TVConfirm()
					{
						BorderColor =
						...
						"スロット " + (selectIndex + 1) + " にセーブします。", "はい", "いいえ") == 0)
					{
						...
						saveDataSlot.MapName = TopViewCommon.GetMapName(TopView.I.Map.MapFile, "Tests/t0001");
