...
using Charlotte.Shootings.SHScripts;
using Charlotte.Shootings.SHScripts.Tests;
using Charlotte.Shootings.SHShots;
using Charlotte.Shootings.SHWalls;

namespace Charlotte.Shootings
{
	public class Shooting : IDisposable
	{
		public SHScript Script = new SHScript_Test何もなし0001(); // 軽量なダミー初期オブジェクト
		...
		public void Perform()
		{
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				{
					if (ShootingConsts.PLAYER_DEAD_FRAME_MAX < ++this.Player.DeadFrame)
					{
						this.Player.DeadFrame = 0;

						if (this.Status.SHZanki <= 0) // ? 残機不足
							break;

						this.システム的な敵クリア();

						this.Status.SHZanki--;
						this.Status.SHAttackLevel = Math.Max(0, this.Status.SHAttackLevel - 1);
						this.Player.RebornFrame = 1;
						goto endDead;
					}
					int frame = this.Player.DeadFrame; // 値域 == 2 ～ GameConsts.PLAYER_DEAD_FRAME_MAX
					double rate = DDUtils.RateAToB(2, ShootingConsts.PLAYER_DEAD_FRAME_MAX, frame);
					...
					if (frame == 2) // init
					{
						...
						DDGround.EL.Add(SCommon.Supplier(SHEffects.PlayerDead(this.Player.X, this.Player.Y)));
					}
				}
			endDead:
				...
				{
					if (ShootingConsts.PLAYER_REBORN_FRAME_MAX < ++this.Player.RebornFrame)
					{
					...
					double rate = DDUtils.RateAToB(2, ShootingConsts.PLAYER_REBORN_FRAME_MAX, frame);
			...
			endReborn:
				...
				{
					if (ShootingConsts.PLAYER_INVINCIBLE_FRAME_MAX < ++this.Player.InvincibleFrame)
					{
						this.Player.InvincibleFrame = 0;
						goto endInvincible;
					}
					int frame = this.Player.InvincibleFrame; // 値域 == 2 ～ GameConsts.PLAYER_INVINCIBLE_FRAME_MAX
					double rate = DDUtils.RateAToB(2, ShootingConsts.PLAYER_INVINCIBLE_FRAME_MAX, frame);
			...
			endInvincible:
				...
				foreach (SHWall wall in this.Walls.Iterate())
					wall.Draw();

				this.Player.Draw();

				foreach (SHEnemy enemy in this.Enemies.Iterate())
				{
				...
				foreach (SHShot shot in this.Shots.Iterate())
				{
						...
						)
					{
						foreach (SHShot shot in this.Shots.Iterate())
						{
							...
							if (
								...
								!(enemy.Kind == SHEnemy.Kind_e.ボス && shot.Kind == SHShot.Kind_e.ボム) // ? ボス x ボム ではない。
								)
							{
								// ★ 敵_被弾ここから
		...
		public DDList<SHWall> Walls = new DDList<SHWall>();
		public DDList<SHEnemy> Enemies = new DDList<SHEnemy>();
		public DDList<SHShot> Shots = new DDList<SHShot>();
		...
		private void Pause()
		{
			...
			for (; ; )
			{
				...
				switch (selectIndex)
				{
					...
					case 1:
						if (new SHConfirm() { BorderColor = new I3Color(200, 0, 0) }.Perform("開始ステージからやり直します。", "はい", "いいえ") == 0)
						{
					...
					case 2:
						if (new SHConfirm() { BorderColor = new I3Color(0, 0, 200) }.Perform("タイトル画面に戻ります。", "はい", "いいえ") == 0)
						{
							this.RequestReturnToTitleMenu = true;
							goto endLoop;
						}
		...
		public void システム的な敵クリア()
		{
			foreach (SHEnemy enemy in this.Enemies.Iterate())
			{
				if (
					enemy.DeadFlag || // ? 敵：死亡
					enemy.Kind == SHEnemy.Kind_e.アイテム ||
					enemy.Kind == SHEnemy.Kind_e.ボス
					)
				{
					// noop
				}
